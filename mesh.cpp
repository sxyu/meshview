...
void Mesh::update(bool force_init) {
    ...
    float* vert_data_ptr; Index* face_data_ptr;
    if (_tex_coords.rows()) {
        n_verts = _tex_coords.rows(); n_faces =_tex_faces.rows();
        ...
        vert_data_ptr = _data_tex.data(); face_data_ptr = _tex_faces.data();
    } else {
        n_verts = data.rows(); n_faces = faces.rows();
        vert_data_ptr = data.data(); face_data_ptr = faces.data();
    }
    ...
    glBufferData(GL_ARRAY_BUFFER, BUF_SZ, (GLvoid*) vert_data_ptr, GL_STATIC_DRAW);

    glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, EBO);
    glBufferData(GL_ELEMENT_ARRAY_BUFFER, INDEX_SZ, (GLvoid*) face_data_ptr, GL_STATIC_DRAW);

    // set the vertex attribute pointers
    // vertex positions
    glEnableVertexAttribArray(0);
    glVertexAttribPointer(0, 3, GL_FLOAT, GL_FALSE, VERT_SZ, (GLvoid*)(POS_OFFSET*SCALAR_SZ));
    // vertex texture coords
    glEnableVertexAttribArray(1);
    glVertexAttribPointer(1, 3, GL_FLOAT, GL_FALSE, VERT_SZ, (GLvoid*)(COLOR_OFFSET*SCALAR_SZ));
    // vertex normals
    glEnableVertexAttribArray(2);
    glVertexAttribPointer(2, 3, GL_FLOAT, GL_FALSE, VERT_SZ, (GLvoid*)(NORMALS_OFFSET*SCALAR_SZ));
    glBindVertexArray(0);
}
...
Mesh Mesh::Triangle(const Eigen::Ref<const Vector3f>& a,
    ...
    Mesh m(3);
    m.data <<
        a[0], a[1], a[2], 0.f, 0.f, 0.f, n[0], n[1], n[2],
        b[0], b[1], b[2], 0.f, 1.f, 0.f, n[0], n[1], n[2],
        c[0], c[1], c[2], 1.f, 1.f, 0.f, n[0], n[1], n[2];
...
Mesh Mesh::Square() {
    ...
    m.faces << 0, 3, 1,
                 1, 3, 2;
    m.data <<
        0.5,  0.5, 0.f,   1.0f, 1.0f, 0.f,        0.0f, 0.0f, 1.0f,
        0.5, -0.5, 0.f,   1.0f, 0.0f, 0.f,        0.0f, 0.0f, 1.0f,
        -0.5, -0.5, 0.f,   0.0f, 0.0f, 0.f,       0.0f, 0.0f, 1.0f,
        -0.5,  0.5, 0.f,   0.0f, 1.0f, 0.f,       0.0f, 0.0f, 1.0f;
    m.shading_type = ShadingType::texture;
    return m;
}
...
Mesh Mesh::Cube() {
    ...
    m.data <<
        ...
        // back
        -0.5, -0.5, -0.5,    0.0f,  0.0f, 0.f,    0.0f,  0.0f, -1.0f,
         0.5,  0.5, -0.5,    1.0f,  1.0f, 0.f,    0.0f,  0.0f, -1.0f,
         0.5, -0.5, -0.5,    1.0f,  0.0f, 0.f,    0.0f,  0.0f, -1.0f,
         0.5,  0.5, -0.5,    1.0f,  1.0f, 0.f,    0.0f,  0.0f, -1.0f,
        -0.5, -0.5, -0.5,    0.0f,  0.0f, 0.f,    0.0f,  0.0f, -1.0f,
        -0.5,  0.5, -0.5,    0.0f,  1.0f, 0.f,    0.0f,  0.0f, -1.0f,
        ...
        // left
        -0.5,  0.5,  0.5,    1.0f,  0.0f, 0.f,   -1.0f,  0.0f,  0.0f,
        -0.5,  0.5, -0.5,    1.0f,  1.0f, 0.f,   -1.0f,  0.0f,  0.0f,
        -0.5, -0.5, -0.5,    0.0f,  1.0f, 0.f,   -1.0f,  0.0f,  0.0f,
        -0.5, -0.5, -0.5,    0.0f,  1.0f, 0.f,   -1.0f,  0.0f,  0.0f,
        -0.5, -0.5,  0.5,    0.0f,  0.0f, 0.f,   -1.0f,  0.0f,  0.0f,
        -0.5,  0.5,  0.5,    1.0f,  0.0f, 0.f,   -1.0f,  0.0f,  0.0f,
         ...
         // bottom
        -0.5, -0.5, -0.5,    0.0f,  1.0f, 0.f,    0.0f, -1.0f,  0.0f,
         0.5, -0.5, -0.5,    1.0f,  1.0f, 0.f,    0.0f, -1.0f,  0.0f,
         0.5, -0.5,  0.5,    1.0f,  0.0f, 0.f,    0.0f, -1.0f,  0.0f,
         0.5, -0.5,  0.5,    1.0f,  0.0f, 0.f,    0.0f, -1.0f,  0.0f,
        -0.5, -0.5,  0.5,    0.0f,  0.0f, 0.f,    0.0f, -1.0f,  0.0f,
        -0.5, -0.5, -0.5,    0.0f,  1.0f, 0.f,    0.0f, -1.0f,  0.0f,
...
Mesh Mesh::Sphere(int rings, int sectors) {
    ...
    for (int r = 0; r < rings - 1; r++) {
        ...
        for (int s = 0; s < sectors; s++) {
            ...
            m.faces.row(fid++) << r * sectors + nx_s,
                                  r *sectors + s,
                                  nx_r * sectors + s;
            m.faces.row(fid++) <<
                     nx_r * sectors + s,
...
// *** PointCloud ***
PointCloud::PointCloud(size_t num_verts) : VAO((Index)-1) {
    data.resize(num_verts, data.ColsAtCompileTime);
    transform.setIdentity();
}
...
BOTH_MESH_AND_POINTCLOUD(set_translation(const Eigen::Ref<const Vector3f>& vec) {
    (transform.topRightCorner<3,1>() = vec);
    return *this;
})
...
BOTH_MESH_AND_POINTCLOUD(apply_transform(const Eigen::Ref<const Matrix4f>& mat) {
