...
int main(int argc, char** argv) {
    ...
    for (int i = 0; i < 256; ++i) {
        for (int j = 0; j < 256; ++j) {
            ...
            mat.block<1, 3>(i, j * 3) <<  i / 255.f,                // r
                                          1.f - i / 255.f,          // g
                                          mat(i, j * 3) = 0.5f;     // b
            mat_spec.block<1, 3>(i, j * 3).setConstant(1.f - (j-128.f) / 128.f);
        }
    }
    auto& cube =
        viewer.add_cube()
    ...
    // * Basic sphere
    auto& uv_sph =
        viewer.add_sphere(Vector3f::Zero(), /* radius */ 0.5f) // use defalt color orange
            .translate(Vector3f(2.f, 0.f, 0.f))
            .set_shininess(32.f);
    ...
    viewer.add_point_cloud(random_pts, /*r,g,b*/ 0.f, 1.0f, 1.0f)
        .rotate(Eigen::AngleAxisf(-M_PI / 2.f, Eigen::Vector3f(1.f, 0.f, 0.f)).toRotationMatrix())
        .scale(1.5f);
    ...
    pyra_vert <<
           -1.f, -1.f, -1.f,
            ...
            -1.f, -1.f, -1.f;
    auto& pyra =
        viewer.add_mesh(pyra_vert,
            /* Triangle indices (unsigned fx3) here; pass empty to use: 0 1 2, 3 4 5 etc */
                Triangles(),
            /*r,g,b*/ 0.f, 1.0f, 1.0f)
    ...
    // * Triangle mesh: textured (tex coords are for demonstration only, not a real uv unwrap)
